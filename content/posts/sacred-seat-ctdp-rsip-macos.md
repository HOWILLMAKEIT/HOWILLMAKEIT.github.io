...
文章提出了两套方法，分别处理「一次具体行为」和「长期状态」两个层面：

### 链式时延协议（CTDP）：守住一次明确的承诺

CTDP 用于解决“开始困难”和“中途放弃”。

1. **选择一个清晰的触发标志**，作为“神圣座位”——例如坐到固定书桌、戴上耳机或发送一条预约消息；
2. **触发即执行**：一旦触发，就只执行事先约定的任务；每完成一次，当前链长增加一次；
3. **边界行为只能二选一**：要么承认本次失败并让链条归零，要么把该行为写成以后永久允许的判例；
4. **失败后不责备自己**，只在失败前重新设计触发条件和规则边界。

CTDP 的重点不是打卡次数，而是让每次触发都保持可信——链条越长，放弃当前承诺的即时成本越高。

### 递归稳态迭代协议（RSIP）：用小规则逐步改变长期状态

RSIP 用于解决长期反复出现的负面稳态，例如熬夜、沉迷手机或长期无法启动学习。

1. **先写下最终想改变的状态**；
2. **沿时间向前回溯**，寻找仍然容易干预的最早节点；
3. **把干预方式写成具体、低阻力、可验证的规则节点**；
4. **先小后大**：上层节点是更容易先做到的小目标，下层节点是它所服务的更大目标；
5. **增量建立**：新节点应建立在已经稳定的节点之上，不追求一次加入很多规则；
6. **失败即回滚**：某个节点长期无法维持时，说明当前路径或加入顺序不合适；回滚并换一条路径，而不是责备自己。
