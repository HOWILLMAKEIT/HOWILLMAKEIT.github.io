...
改造前，Skills 管理页只展示已安装列表。当本地只有十几个 Skill 时，滚动浏览已经够用；但数量增加到上百个后，查找效率会明显下降：记得名称时还可以按首字母定位，只记得功能时则需要逐项阅读描述。
...
要支持正文搜索，还需要解决一个数据问题：**前端原有数据里并没有 `SKILL.md` 正文。**
...
页面不需要区分正文来自数据库还是文件系统，只依赖 `Skill ID → 正文` 接口。
...
正文缓存的新鲜期设为 30 秒：用户连续输入时不会反复读取磁盘，长时间打开页面也不会一直使用旧数据。安装、卸载、更新、导入或恢复 Skill 的 Mutation 完成后，还会主动使查询失效：
...
正文通过三种方式更新：30 秒后自然过期、应用内操作后主动失效，以及外部编辑后的窗口聚焦刷新。

## React：实现搜索范围过滤

页面只维护两个状态：搜索词 `searchQuery` 和搜索范围 `searchScope`。这里没有建立倒排索引，也不做分词和相关性排序；实现本质上是不区分大小写的内存子串匹配。

过滤前先去除查询词两端的空格，并统一转换为小写。这里使用不跟随操作系统语言的 `toLowerCase()`；如果使用 `toLocaleLowerCase()`，在土耳其语或阿塞拜疆语系统中，英文大写 `I` 会变成 `ı`，从而让 `Installed` 无法被 `installed` 匹配：
...
测试需要确认搜索结果、字段范围和缓存刷新行为。搜索过滤测试位于 `tests/components/UnifiedSkillsPanel.test.tsx`，缓存刷新测试位于 `tests/hooks/useSkills.test.tsx`。它们共同覆盖：
...
3. 使用短期缓存、显式失效和窗口聚焦刷新减少重复磁盘读取，并及时更新正文；
4. 将搜索范围定义为可测试的字段边界；
5. 遇到非法目录或单个文件读取失败时跳过对应内容，不影响整个页面。

搜索逻辑本身比较简单，主要工作集中在正文读取、缓存刷新、搜索字段定义和错误处理。上述实现使名称、描述和正文搜索保持一致，并在文件发生变化或部分内容读取失败时继续提供可用结果。
